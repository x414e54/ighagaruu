...
CModelViewerCamera::CModelViewerCamera()
{
    ...
    m_vModelCenter = D3DXVECTOR3(0,0.6f,0);
    m_fRadius    = 5.0f;
...
//--------------------------------------------------------------------------------------
VOID CModelViewerCamera::FrameMove( FLOAT fElapsedTime, FLOAT* g_AvatarOrientation )
{
    if( IsKeyDown(m_aKeys[CAM_RESET]) )
        Reset();
    ...
    GetInput( m_bEnablePositionMovement, m_nCurrentButtonMask != 0, true,true );
    ...
    if( m_ViewArcBall.IsBeingDragged() && m_bMouseRButtonDown )
    {
	        // Update the pitch & yaw angle based on mouse movement
        float fPitchDelta = m_vRotVelocity.y;
        float fYawDelta   = m_vRotVelocity.x;
        ...
        m_fCameraPitchAngle += fPitchDelta;
		if (m_fCameraPitchAngle>1.5) { m_fCameraPitchAngle=1.5;}
		if (m_fCameraPitchAngle<0) { m_fCameraPitchAngle=0;}
		float test=*g_AvatarOrientation-fYawDelta;
		*g_AvatarOrientation   = test;
	}

		if( m_ViewArcBall.IsBeingDragged() && m_bMouseLButtonDown )
    {
	        // Update the pitch & yaw angle based on mouse movement
        float fYawDelta   = m_vRotVelocity.x;
        float fPitchDelta = m_vRotVelocity.y;
