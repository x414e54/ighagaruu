...
CDXUTDialog             g_TopUI;                  // dialog for all controls
CDXUTDialog             g_TargetUI;                  // dialog for all controls
CDXUTDialog             g_LoginUI; 
CDXUTDialog             g_ScoreUI; 
CDXUTDialog             g_OptionsUI;                  // dialog for options
CDXUTDialog             g_CharacterUI;             // dialog for character controls
CDXUTDialog             g_SpellInfoUI;             
CDXUTDialog             g_CastBarUI;
CDXUTDialog				g_CharacterChooseUI;
CDXUTDialog				g_RedTextUI;
CDXUTScrollBar			g_ChatScroll;
CDXUTDialog				g_LoginPopUp;
...
void    CharacterFrameMove( double fTime, float fElapsedTime, void* pUserContext );
...
void    IntroFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext );
...
INT WINAPI wWinMain( HINSTANCE, HINSTANCE, LPWSTR, int )
{
	...
	DXUTCreateDevice( true, 1920, 1200 );
	pMusic->Reset();
...
{
	if (myChar!=NULL) {
		CharacterFrameMove(fTime, fElapsedTime, pUserContext );
		g_Camera.FrameMove( fElapsedTime, &myChar->pos.orientation );
	}
}
...
void CharacterFrameMove(double fTime, float fElapsedTime, void* pUserContext ) {
	...
	if (g_bFoward || g_bBackward || g_bRight || g_bLeft) {
		...
		newX=myChar->pos.x+vAvatar.x;
		newZ=myChar->pos.z+vAvatar.z;
	...
	for (int i=0; i<11; i++) {
		if (g_CharacterUI.GetButton(IDC_SPELLCAST1+i)->m_bMouseOver) {
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	{
		if (myChar!=NULL) {
			CharacterFrameRender(pd3dDevice, fTime, fElapsedTime, pUserContext );
		} else {
			if (loggedin) {
				ChooseFrameRender(pd3dDevice, fTime, fElapsedTime, pUserContext );
			} else {
				IntroFrameRender(pd3dDevice, fTime, fElapsedTime, pUserContext );
			}
		}
	}
...
void IntroFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext ) {
	...
	g_pEffect->End();
	V( g_LoginUI.OnRender( fElapsedTime ) );
	V( g_LoginPopUp.OnRender( fElapsedTime ) );
}
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext )
{
	...
	*pbNoFurtherProcessing = g_DialogResourceManager.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (myChar!=NULL) {
		*pbNoFurtherProcessing = g_TopUI.MsgProc( hWnd, uMsg, wParam, lParam );
		if( *pbNoFurtherProcessing )
			return 0;
		*pbNoFurtherProcessing = g_OptionsUI.MsgProc( hWnd, uMsg, wParam, lParam );
		if( *pbNoFurtherProcessing )
			return 0;
		*pbNoFurtherProcessing = g_CharacterUI.MsgProc( hWnd, uMsg, wParam, lParam );
		if( *pbNoFurtherProcessing )
			return 0;
		...
		if (loggedin) {
			*pbNoFurtherProcessing = g_CharacterChooseUI.MsgProc( hWnd, uMsg, wParam, lParam );
			if( *pbNoFurtherProcessing )
				return 0;
		} else {
			*pbNoFurtherProcessing = g_LoginPopUp.MsgProc( hWnd, uMsg, wParam, lParam );
			if( *pbNoFurtherProcessing )
				return 0;
			*pbNoFurtherProcessing = g_LoginUI.MsgProc( hWnd, uMsg, wParam, lParam );
			if( *pbNoFurtherProcessing )
				return 0;
		}
	}
...
void CharacterGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext ) {
	...
	case IDC_EDITBOX1: {
		LPCWSTR currentText = g_CharacterUI.GetEditBox(IDC_EDITBOX1)->GetText();
		const wchar_t* currentText2 = (const wchar_t*)currentText;
		wchar_t firstChar = currentText[0];
		switch (firstChar)
		{
		case L'/':
			{
				Tokenizer* toke = new Tokenizer(currentText2+1, L' ');
				std::wstring next=toke->getNext();
				const wchar_t* cwnext=next.c_str();
	...
	if (iResult > 0) {
		...
		switch (firstByte) {
			case 24:   {  //stats
				...
				pMusic->Stop();
				D3DXVECTOR3 vecEye(0.0f, 1.2f, 5.0f);
				D3DXVECTOR3 vecAt (0.0f, -yadjust[myChar->info.classs+(myChar->info.faction*4)], -0.0f);
				g_Camera.SetViewParams( &vecEye, &vecAt );
				g_Camera.SetRadius(5.0f,1.0f,10.0f);
				DWORD threadId;
				sendbuf[0]=0;
...
void SetSpells() {
	...
	for (i=0; i<11; i++) {
		if (myChar->inv.spells[i]!=NULL) {
			RECT current ={g_CharacterUI.GetButton( IDC_SPELLCAST1+i )->GetElement(0)->rcTexture.left, g_CharacterUI.GetButton( IDC_SPELLCAST1+i )->GetElement(0)->rcTexture.top, g_CharacterUI.GetButton( IDC_SPELLCAST1+i )->GetElement(0)->rcTexture.right, g_CharacterUI.GetButton( IDC_SPELLCAST1+i )->GetElement(0)->rcTexture.bottom};
...
void SetInventory() {
	...
	for (i=0; i<6; i++) {
		if (myChar->inv.gear[i]!=NULL) {
			RECT current ={g_CharacterUI.GetButton( IDC_HEAD+i )->GetElement(0)->rcTexture.left, g_CharacterUI.GetButton( IDC_HEAD+i )->GetElement(0)->rcTexture.top, g_CharacterUI.GetButton( IDC_HEAD+i )->GetElement(0)->rcTexture.right, g_CharacterUI.GetButton( IDC_HEAD+i )->GetElement(0)->rcTexture.bottom};
	...
	for (i=0; i<6; i++) {
		if (myChar->inv.items[i]!=NULL) {
			RECT current ={g_CharacterUI.GetButton( IDC_INV1+i )->GetElement(0)->rcTexture.left, g_CharacterUI.GetButton( IDC_INV1+i )->GetElement(0)->rcTexture.top, g_CharacterUI.GetButton( IDC_INV1+i )->GetElement(0)->rcTexture.right, g_CharacterUI.GetButton( IDC_INV1+i )->GetElement(0)->rcTexture.bottom};
